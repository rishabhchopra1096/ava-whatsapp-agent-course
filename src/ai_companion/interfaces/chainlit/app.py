"""
💬 CHAINLIT WEB INTERFACE - Ava's demo and development chat interface

WHAT IS THIS FILE?
This is Ava's web-based chat interface built with Chainlit - perfect for:
- Testing Ava during development
...
- Real-time streaming responses (watch Ava think!)
...
3. SAME Ava brain processes (graph.py + nodes.py)
...
- graph_builder: Uses IDENTICAL workflow as WhatsApp (same Ava brain)
...
3. Chat with Ava exactly like production, but with rich UI!
...
Each handler follows Ava's standard processing pattern:
1. Get user input (text from typing, image from upload, or audio from recording)
2. Send input through Ava's identical LangGraph brain (same workflow as WhatsApp production)
3. Display Ava's response in web interface (text, image, or audio player)
"""
...
# Think of it like a clipboard that temporarily holds file contents while Ava works on them
...
# Like having a pre-built chat website that we just need to connect Ava's brain to
...
# Same format used in WhatsApp interface, so Ava's brain processes messages identically
from langchain_core.messages import AIMessageChunk, HumanMessage

# AsyncSqliteSaver is LangGraph's conversation memory system that uses SQLite database files.
# This system stores conversation history so Ava remembers what you talked about previously.
...
# AVA'S CORE BRAIN COMPONENTS - The same AI modules used in WhatsApp interface

# graph_builder is Ava's complete workflow system that acts like a conductor orchestrating all AI modules.
...
# These are Ava's AI processing modules for different media types:
# ImageToText is Ava's vision system that analyzes photos users upload via web interface.
# SpeechToText is Ava's hearing system that transcribes voice recordings to text for processing.
# TextToSpeech is Ava's speaking system that converts Ava's text responses to audio files.
from ai_companion.modules.image import ImageToText
from ai_companion.modules.speech import SpeechToText, TextToSpeech

# settings is Ava's configuration file that contains API keys, model choices, and database paths.
...
# Converts audio recordings (voice messages) into text that Ava's LangGraph brain can understand
...
async def on_chat_start():
    ...
    # thread_id = unique identifier for this user's conversation with Ava
    ...
    # This shows how Ava's memory system works - if User A says "I'm John", 
...
async def on_message(message: cl.Message):
    ...
    3. Process through IDENTICAL Ava brain (graph.py workflow)
    ...
    Users see Ava "typing" her response word-by-word, like ChatGPT.
    """
    ...
    # This is like creating a blank text box that we'll fill as Ava "types" her response
    ...
    # Example: if user typed "Hello Ava!", content = "Hello Ava!"
    ...
    # STEP 4: PROCESS THROUGH AVA'S BRAIN (IDENTICAL TO WHATSAPP PROCESSING)
    # Now we have the content text (+ image analysis), send it through Ava's complete LangGraph workflow
    ...
    # Like showing a spinning wheel while Ava processes the message
    async with cl.Step(type="run"):
        ...
        async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
            ...
            # STREAM AVA'S RESPONSE IN REAL-TIME (UNIQUE TO CHAINLIT)
            # Unlike WhatsApp (store-and-forward), Chainlit supports real-time streaming
            # Users see Ava "typing" her response word-by-word, like ChatGPT
            
            # PROCESS MESSAGE AND STREAM RESPONSE
            # async for = loop that processes streaming data as it arrives
            # graph.astream() = stream Ava's processing in real-time (vs ainvoke() which waits for completion)
            async for chunk in graph.astream(
                ...
                # chunk = each piece of streaming data from Ava's workflow
                ...
                if chunk[1]["langgraph_node"] == "conversation_node" and isinstance(chunk[0], AIMessageChunk):
                    ...
                    # chunk[0].content = the actual text content from Ava's response
                    await msg.stream_token(chunk[0].content)

            # GET AVA'S FINAL STATE AFTER PROCESSING COMPLETES
            # After streaming completes, we need to see Ava's final decisions (text/image/audio response)
    ...
    # output_state.values.get("workflow") = get router_node's decision from Ava's final state
    # "audio" = router_node decided Ava should respond with voice
    if output_state.values.get("workflow") == "audio":
        
        # EXTRACT AUDIO RESPONSE DATA FROM AVA'S STATE
        # output_state.values["messages"] = list of all messages in conversation
        # [-1] = get the last message (Ava's response)
        # .content = the actual text content of Ava's response
        response = output_state.values["messages"][-1].content
        
        # output_state.values["audio_buffer"] = the actual audio file data (bytes)
        # This is Ava's voice saying the response text (generated by audio_node using ElevenLabs)
        ...
        # content=response = Ava's text (user can read while listening)
    ...
    # IMAGE RESPONSE - AVA WANTS TO SEND A GENERATED IMAGE
        ...
        # EXTRACT IMAGE RESPONSE DATA FROM AVA'S STATE
        # output_state.values["messages"][-1].content = Ava's caption text for the image
        response = output_state.values["messages"][-1].content
        ...
        # await = wait for message to be sent to user's browser
        # content=response = Ava's caption text
...
async def on_audio_end(elements):
    ...
    2. Processes through Ava's brain (same logic as text messages)  
    3. Generates voice response (full voice conversation)
    
    THE VOICE-TO-VOICE PIPELINE:
    User Voice → Transcription → Ava's Brain → Text Response → Voice Synthesis → Audio Reply
    ...
    # cl.Message() = creates a message in the chat interface
    # author="You" = label this message as coming from the user (not Ava)
    ...
    # Convert the user's voice recording into text that Ava's brain can process
    
    # PROCESS AUDIO THROUGH AVA'S HEARING SYSTEM
    ...
    # STEP 4: PROCESS TRANSCRIPTION THROUGH AVA'S BRAIN (IDENTICAL TO TEXT PROCESSING)
    ...
    # SETUP CONVERSATION MEMORY AND PROCESS THROUGH AVA'S WORKFLOW
    ...
    async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
        ...
        # await = wait for Ava's complete processing to finish
    ...
    # CREATE AUDIO ELEMENT FOR AVA'S VOICE RESPONSE
    # cl.Audio() = creates an audio player widget in the web interface
    output_audio_el = cl.Audio(
        name="Audio",                    # Label shown in the web interface
        auto_play=True,                  # Start playing immediately when message appears
    ...
    # await = wait for message to be sent to user's browser
    # cl.Message() = creates a message in the chat interface
    # content=response_text = Ava's text response (user can read while listening)
    # elements=[output_audio_el] = attach the audio player to this message
