...
SIMPLIFIED FLOW MAP (ReAct preparation - memory nodes removed):
                    START
            ...
            ┌─────────┼─────────┐
            ↓         ↓         ↓
    conversation_node  image_node  audio_node  voice_calling_node
            ↓         ↓         ↓              ↓
    (should_summarize) (should_summarize) (should_summarize)    END
            ↓         ↓         ↓
        summarize_conversation_node  OR  END
                      ↓ (add_edge - always after summarize)
                     END

REAL EXAMPLES (simplified paths - ready for ReAct conversion):

EXAMPLE 1 - Text Response, Short Chat:
You: "How are you?" (5 messages total in chat)
Route: START → router_node → conversation_node → END (no summarize needed)

EXAMPLE 2 - Image Response, Long Chat:  
You: "What are you up to?" (35 messages total in chat)
Route: START → router_node → image_node → summarize_conversation_node → END

EXAMPLE 3 - Voice Call:
You: "Can you call me back?" 
Route: START → router_node → voice_calling_node → END (always direct to end)
"""
...
def create_workflow_graph():
    ...
    # STEP 2: Register all the worker nodes (SIMPLIFIED - memory nodes removed)
    ...
    # Router Node - entry point for all conversations
    # - What it does: "How should I respond to this?"
    # - Real example: You ask "What are you up to?" → decides "I should send a picture"
    # - Why it exists: Makes Pepper autonomous - she chooses text/image/audio/voice responses
    ...
    graph_builder.add_node("voice_calling_node", voice_calling_node)           # Handle phone call requests
    ...
    # STEP 3: Define the execution flow - SIMPLIFIED (ready for ReAct conversion)
    ...
    # SIMPLIFIED FLOW: START → router → response_nodes → summarize/END
    
    # Entry point: every conversation starts with routing decision
    graph_builder.add_edge(START, "router_node")
    
    # BRANCH 1: Route to appropriate response generator
    # Router decides: "conversation_node" OR "image_node" OR "audio_node" OR "voice_calling_node"
    ...
    # After response nodes complete, check if chat is getting too long
    ...
    # Voice calling always ends conversation (call is initiated, WhatsApp message sent)
