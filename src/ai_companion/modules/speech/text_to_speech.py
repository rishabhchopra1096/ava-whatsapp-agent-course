...
# Provides consistent error handling across Pepper's text-to-speech system
from ai_companion.core.exceptions import TextToSpeechError

# Pepper's centralized configuration containing API keys, voice IDs, and model names
...
class TextToSpeech:
    """
    🗣️ pepper'S SPEAKING SYSTEM - Pepper's ability to speak text responses as realistic voice
    
    WHAT IT DOES:
    This class gives Pepper the ability to "speak" her text responses out loud as realistic
    audio that sounds like a real person talking. When Pepper wants to send voice messages
    instead of text, this system converts her written responses into actual speech.
    
    HOW IT WORKS (USER STORY):
    1. Pepper's LangGraph brain generates text response: "I'm working on ML optimization at Groq today"
    ...
    4. User receives Pepper's actual voice saying the response as a voice message
    5. Consistent personality: Always sounds like the same "Pepper" voice
    
    REAL-WORLD ANALOGY:
    This is like Pepper having a beautiful, consistent speaking voice that she can use
    ...
    - Maintains consistent voice identity across all of Pepper's speech
    - Professional quality suitable for production AI applications
    
    TECHNICAL APPROACH:
    - Uses specific voice ID to ensure Pepper always sounds the same
    - Configurable voice settings for optimal naturalness
    - Handles audio generation as streaming bytes for efficient processing
    
    USED BY:
    - audio_node in LangGraph: When Pepper decides to respond with voice
    - Voice conversations: In Chainlit's voice chat feature
    - WhatsApp voice responses: When appropriate for conversation context
    
    INTEGRATION WITH pepper'S PERSONALITY:
    The voice becomes part of Pepper's identity - users will recognize her voice
    ...
    # ELEVENLABS_VOICE_ID: Specific voice identity that defines how Pepper sounds
    REQUIRED_ENV_VARS = ["ELEVENLABS_API_KEY", "ELEVENLABS_VOICE_ID"]

    def __init__(self):
        """
        🔧 INITIALIZE pepper'S SPEAKING SYSTEM - Set up voice synthesis capabilities
        ...
        The voice ID determines how Pepper sounds - her vocal identity.
        Without it, Pepper might sound different each time she speaks.
        With it, she maintains a consistent, recognizable voice personality.
        
        REAL-WORLD ANALOGY:
        This is like configuring Pepper's vocal cords to always sound like the same person.
        Just like you recognize your friend's voice on the phone, users will recognize Pepper's voice.
        """
    ...
    def _validate_env_vars(self) -> None:
        ...
        - ELEVENLABS_VOICE_ID: Specifies which voice Pepper should use (her vocal identity)
        ...
        - Missing voice ID: Pepper might sound different each time (inconsistent personality)
        """
    ...
    def client(self) -> ElevenLabs:
        ...
        instead of setting up a new recording session every time Pepper wants to speak.
    ...
    async def synthesize(self, text: str) -> bytes:
        """
        🎤 VOICE SYNTHESIS ENGINE - Converts Pepper's text responses into realistic speech
        
        WHAT IT DOES:
        Takes Pepper's written response and converts it into realistic speech audio that sounds
        like a real person talking. This is how Pepper "speaks" her responses as voice messages.
        
        HOW IT WORKS (USER STORY):
        1. Pepper's brain generates text: "I'm currently optimizing ML models at Groq"
        2. audio_node calls this method to convert text to speech
        3. ElevenLabs AI creates realistic voice audio of Pepper saying those words
        4. Returns audio bytes that can be sent as voice message to user
        5. User hears Pepper's consistent, natural-sounding voice speaking the response
        
        PARAMETERS:
        text: The text response that Pepper wants to speak out loud
              Example: "Hi! I'm working on some interesting ML projects today."
        
        RETURNS:
        Audio bytes (binary data) containing Pepper's voice speaking the text
        These bytes can be sent as voice messages via WhatsApp or played in Chainlit
        
        REAL-WORLD ANALOGY:
        This is like Pepper reading her written response out loud with her natural voice.
        ...
        try:
            ...
            audio_generator = self.client.generate(
                # TEXT: The actual words for Pepper to speak
                text=text,
                
                # VOICE CONFIGURATION: Defines how Pepper sounds
                voice=Voice(
                    # VOICE_ID: Specific voice identity that makes Pepper sound consistent
                    # settings.ELEVENLABS_VOICE_ID comes from environment variables
                    # This ensures Pepper always sounds like the same person
            ...
            # Contains Pepper's voice speaking the input text with natural intonation
            return audio_bytes
        ...
        # except Exception catches any error that occurred during voice synthesis
            ...
            # TextToSpeechError provides consistent error handling across Pepper's system
